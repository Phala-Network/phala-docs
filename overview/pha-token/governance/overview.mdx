...
The public referenda chamber is one of the three bodies of on-chain governance as it's instantiated in Phala. The other two bodies are the council and the technical committee.
...
This guide will instruct token holders how to propose and vote on public referenda using the Democracy module as it's implemented in Phala.
...
To vote on a referendum, navigate to the "Democracy" tab of Phala. Any active referendum will show in the "referenda" column. Click the blue button "Vote" to cast a vote for the referendum.
...
You can do this from the "Accounts" page in [Phala](https://polkadot.js.org/apps/?rpc=wss%3A%2F%2Fphala.api.onfinality.io%2Fpublic-ws#/accounts), unless you use Ledger. First check that your account has a “democracy” lock by opening the details on your balance. In the example below the account has 9 PHA locked in democracy.
