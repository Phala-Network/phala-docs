...
- **For on-chain governance mode:**
  ...
  - Deployed smart contracts (DstackKms, DstackApp) — see [Deploy On-chain KMS Smart Contracts](/dstack-cloud/deploy-onchain-kms)
...
Follow the same steps as [Run a Workload on GCP](/dstack-cloud/run-on-gcp#step-1-configure-dstack-cloud) to set up your global config and GCS bucket.

---

## Step 2: Pull the OS Images

Follow the same steps as [Run a Workload on GCP](/dstack-cloud/run-on-gcp#step-2-pull-the-os-image) to download the `dstack-cloud-0.6.0` images.
...
- [Register Workload Measurements](/dstack-cloud/register-enclave-measurement)
- [KMS and Key Delivery](/dstack-cloud/kms-and-key-delivery)
- [Attestation Integration](/dstack-cloud/attestation-integration)
- [Deploy On-chain KMS Smart Contracts](/dstack-cloud/deploy-onchain-kms)
