...
dstack-cloud is an extension of the [dstack framework](https://github.com/Dstack-TEE/dstack) that enables deploying confidential Docker applications on **GCP Confidential VMs** and **AWS Nitro Enclaves**.
...
2. `dstack-cloud deploy` builds a launch configuration and deploys a TEE environment.

### On GCP:

3. A **dstack CVM** — a Confidential Virtual Machine running dstack OS — is launched with the provided configuration.
4. The CVM boots as a Confidential VM with Intel TDX enabled, then pulls your container images from the Docker registry.
