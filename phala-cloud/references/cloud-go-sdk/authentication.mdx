...
The SDK sends an API version header with every request. The default is `"2026-06-23"`. Override it if you need an older response shape.
...
The SDK sends a `User-Agent` header like `phala-cloud-sdk-go/0.2.0` by default. Override it to identify your application in server logs.
