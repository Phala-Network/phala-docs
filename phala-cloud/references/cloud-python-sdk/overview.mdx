...
| `version` | `ApiVersion` | `"2026-06-23"` | API version to use |
...
The SDK supports four API versions. The version affects response shapes for endpoints such as `get_current_user()` and `get_cvm_list()`.

```python
from phala_cloud import SUPPORTED_API_VERSIONS, DEFAULT_API_VERSION

print(SUPPORTED_API_VERSIONS)
# ("2025-10-28", "2026-01-21", "2026-05-22", "2026-06-23")
print(DEFAULT_API_VERSION)  # "2026-06-23"
```
...
| `get_cvm_status_batch(request)` | Get typed status and resource usage for multiple CVMs |
...
| `create_app_instance(request)` | Create another instance for an app |
...
| Method | Description |
|--------|-------------|
| `list_kms_contracts()` | List KMS contracts |
| `get_kms_contract(slug)` | Get a KMS contract by slug or contract ID |
| `list_kms_contract_nodes(slug)` | List node replicas under a KMS contract |
| `get_kms_info(request)` | Get legacy node-centric KMS info (deprecated) |
| `get_kms_list(request?)` | List legacy node-centric KMS instances (deprecated) |
| `get_kms_on_chain_detail(request)` | Get on-chain KMS details and verification anchors |
...
| `get_compose_hash(...)` | Compute the hash of canonicalized `app-compose.json` |
| `get_mr_config_id(...)` | Compute V2 `mr_config_id` from byte inputs |
| `get_mr_config_id_hex(...)` | Compute V2 `mr_config_id` from hex inputs |
| `get_mr_config_id_v1(...)` | Compute the V1 compose-hash-only format |
| `verify_mr_config_id(...)` | Verify a V2 `mr_config_id` |
