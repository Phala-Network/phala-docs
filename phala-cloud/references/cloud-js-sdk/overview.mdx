...
| `version` | `ApiVersion` | `"2026-06-23"` | [API version](/phala-cloud/references/cloud-js-sdk/api-versioning) to use |
...
| `createAppInstance(request)` **V** | Create another instance for an app |
...
| `getCvmCreateResources()` | Get the CVM creation resource graph |
...
| Method | Description |
|--------|-------------|
| `listKmsContracts(request?)` | List KMS contracts |
| `getKmsContract(request)` | Get a KMS contract by slug or contract ID |
| `listKmsContractNodes(request)` | List node replicas under a KMS contract |
| `getKmsInfo(request)` | Get legacy node-centric KMS info (deprecated) |
| `getKmsList(request?)` | List legacy node-centric KMS instances (deprecated) |
| `getKmsOnChainDetail(request)` | Get on-chain KMS details and verification anchors |
...
Import measurement helpers from `@phala/cloud`:

| Function | Description |
|----------|-------------|
| `getMrConfigId(input)` | Compute a V2 `mr_config_id` from the compose hash, app ID, and key provider |
| `getMrConfigIdV1(composeHash)` | Compute the V1 compose-hash-only format |
| `verifyMrConfigId(mrConfigId, input)` | Verify a V2 `mr_config_id` |

See [KMS](/phala-cloud/references/cloud-js-sdk/kms) for examples.
