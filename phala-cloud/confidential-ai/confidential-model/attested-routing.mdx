...
```bash
curl -s "https://inference.phala.com/v1/aci/sessions?model=phala/qwen3.5-27b" \
  -H "Authorization: Bearer <API_KEY>" \
  | jq -r '.[] | "\(.session_id)  \(.provider)  \(.endpoint)"'
```

Each entry carries a `session_id` of the form `as_<64 hex characters>`, the verifier that produced it, its channel binding, and an `expires_at` timestamp. `?provider=` filters the same list by provider. See [Get Session](/phala-cloud/confidential-ai/confidential-model/api-reference/sessions).
...
{
  "error": {
    "message": "upstream verification failed: none of the requested attested sessions is available for model phala/qwen3.5-27b",
    "type": "service_unavailable"
  }
}
```
...
```json
{
  ...
  "result": "verified",
  "required": true,
  "session_id": "as_941d3d5f...",
  "channel_bindings": [{ "type": "tls_spki_sha256", "value": "4088be15..." }],
  "claims": {
    "tee_attested": { "status": "Asserted", "source": "HardwareProven" }
  }
}
```
