---
description: Secure LLM inference API running in GPU TEE for confidential AI computing.
title: Use AI API
---

## Overview

Phala Cloud provides a simple, powerfull and efficiency API to help you access majory LLM through a unified , which could help you use hassle-free integration using its OpenAI-compatible format. Deploy your application seamlessly across cloud, on-premises, or edge environments.

Phala Cloud provides various LLM inference API. By navigating to **Dashboard -> Confidential AI API** page, you will see details.

## Enable API Service

In order to use the API, **user needs to deposit at least $5 in advance to enable Confidential AI API**. If your balance is not sufficient, you can head to **Dashboard**->**Billing** page then click **Deposit** button to top-up with your bank card or crypto wallet.
...
At the bottom of the page, click the **Enable** button and **Create New API Key** to generate your API key. Copy the key to use when you interact with the API key.

<Frame>
    <img src="/images/confidential-ai/confidential-model/api-keys.png" alt="GPU TEE API Generate Key" />
</Frame>

Now we support private AI inference. It currently supports 6+ models that are running in GPU TEE, you can view them in the models page by clicking the **GPU TEE** checkbox:
...
```shell Confidential Inference Request
curl -X 'POST' \
...
You can verify if the LLM is running in GPU TEE. This can be done by verifying its attestation report. To get the attestation report of the LLM inference, you can do this by sending a POST request to the API endpoint like below:
