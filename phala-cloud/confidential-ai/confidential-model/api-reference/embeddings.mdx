...
|-------|------|----------|-------------|
| `model` | string | Yes | Embedding model ID. Examples: `qwen/qwen3-embedding-8b`, `sentence-transformers/all-minilm-l6-v2`. |
| `input` | string or array | Yes | Input text or list of inputs to embed. |
| `encoding_format` | string | No | Embedding encoding format. Common values are `float` and `base64`. |
| `dimensions` | integer | No | Requested output dimensions, when supported by the selected model. |
