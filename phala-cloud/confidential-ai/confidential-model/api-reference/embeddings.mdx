...
<ParamField body="provider" type="object">
  Routing constraints for this request.

  Set `provider: {"zdr": true}` to route only to serving routes that operate under zero data retention. If no route for the selected embedding model satisfies the constraint, the request fails before input text is sent. See [Zero Data Retention](/phala-cloud/confidential-ai/confidential-model/zero-data-retention).
</ParamField>
