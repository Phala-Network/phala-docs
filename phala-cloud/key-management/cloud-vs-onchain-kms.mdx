...
In Cloud KMS, Phala controls what code runs. If Phala's systems are compromised or pressured, policies can change. With Onchain KMS, governance rules are enforced by the blockchain network—thousands of nodes must agree to execute the smart contract logic. No single entity can override these rules.
...
<AccordionGroup>
  <Accordion title="Ethereum">
    ...
    | RPC | https://kms.dstack-eth-prod5.phala.network <br/> https://kms.dstack-eth-prod7.phala.network <br/> https://kms.dstack-eth-prod9.phala.network |
  ...
  <Accordion title="Base">
    ...
    | RPC | https://kms.dstack-base-prod5.phala.network <br/> https://kms.dstack-base-prod7.phala.network <br/> https://kms.dstack-base-prod9.phala.network |
...
When using Onchain KMS, any change that affects the compose hash (Docker Compose file, pre-launch script, env keys, visibility settings) requires on-chain registration before the update can proceed.

### Standard Flow (Single Owner)

If you own the private key of the DstackApp contract owner, the CLI handles the on-chain registration automatically:

```bash
phala deploy --cvm-id app_abc123 -c docker-compose.yml \
  --kms base --private-key <key> --rpc-url <url>
```

### Multisig Flow (Two-Phase)

When the contract owner is a multisig wallet (e.g., Gnosis Safe), the on-chain transaction requires multiple signatures. The CLI supports a two-phase flow:
...
This outputs:
- The new compose hash (with `0x` prefix for direct use)
- On-chain status (whether device and compose hash are already registered)
- A commit token (valid 7 days)
- A web UI link for manual confirmation
- An API commit URL for programmatic use

**Multisig approval** — use the compose hash to call `addComposeHash(bytes32)` on the DstackApp contract through your multisig wallet.

**Phase 2 — Commit:**

```bash
phala deploy --cvm-id app_abc123 --commit --token <commit-token>
```

Or commit via the dashboard link, or directly via the API:
...
You can configure [webhooks](/phala-cloud/references/webhooks) to receive `cvm.update.pending_approval` events. The webhook payload includes the compose hash, commit token, and commit URL — useful for integrating with CI/CD pipelines or notification systems like Slack.
