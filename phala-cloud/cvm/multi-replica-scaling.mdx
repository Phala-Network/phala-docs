...
Each replica gets its own CVM ID, endpoint URL, and TEE attestation. Traffic is not automatically load-balanced between replicas, so you'll need an external load balancer or DNS-based routing if you want to distribute requests.
