...
The `TARGET_ENDPOINT` tells dstack-ingress where to forward incoming traffic:
...
In the Phala Cloud dashboard, add these secure environment variables:

- `CLOUDFLARE_API_TOKEN`: Your API token from Step 1
- `CERTBOT_EMAIL`: Your email for Let's Encrypt notifications

For other DNS providers:
- Linode: Use `LINODE_API_TOKEN` instead
- Namecheap: Use `NAMECHEAP_USERNAME`, `NAMECHEAP_API_KEY`, and `NAMECHEAP_CLIENT_IP`
...
- CAA records restrict certificate issuance to your specific Let's Encrypt account
...
- [Learn about domain attestation](/phala-cloud/networking/security)
- [Set up gRPC with custom domain](/phala-cloud/networking/how-to/setup-grpc-service)