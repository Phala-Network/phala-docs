...
### Resolves Everywhere Except Android
...
Android rejects a target whose first label starts with an underscore — the old
`_.<gateway-domain>` form. Set `GATEWAY_DOMAIN=gateway.${DSTACK_GATEWAY_DOMAIN}`
and redeploy.
...
/opt/homebrew/bin/openssl s_client -quiet -connect %h:%p -servername %h -verify_hostname %h -verify_return_error
```
